...
st.subheader("General Info",anchor=False)
...
with column[1]:
    ...
    tile2.write(f'**Highest Population**: **{int(np.max(df["Area Population"]))}** people')
...
with column[2]:
    ...
    tile3.write(f'**Highest Price**: **US${round(np.max(df["Price"]),2):,.2f}**')
...
st.subheader('Advanced Search and Comparisons', anchor=False, divider=True)
...
with tile_btns:
    st.session_state.states_selected = tile_btns.multiselect('Choose the states', sorted(abbr_state))
